...
class J1 extends Phaser.GameObjects.Sprite{
    constructor(scene, x, y, key, nombre, derecha,izquierda,saltar,defender,rapido,pesado, interaccion){
        ...
        //[[[CONTROLES]]]
        this.izquierdaTecla     = scene.input.keyboard.addKey(izquierda);
        this.derechaTecla       = scene.input.keyboard.addKey(derecha);
        this.saltarTecla        = scene.input.keyboard.addKey(saltar);
        this.defensaTecla       = scene.input.keyboard.addKey(defender);

        this.ataqueRapidoTecla = scene.input.keyboard.addKey(rapido);
        this.ataquePesadoTecla = scene.input.keyboard.addKey(pesado);
        this.interaccionTecla  = scene.input.keyboard.addKey(interaccion);
    ...
    checkMuerte(){
        if(this.getVida() <= 0){
            if(this.nombre === 'J2'){
                mensajeVictoriaJ1.visible = true;
            } else if (this.nombre === 'J1'){
                mensajeVictoriaJ2.visible = true;
            }
    ...
    jugadorMovimiento(){
        if(!this.getKnockeado()){
            if(this.defensaTecla.isUp){
                this.setDefendiendo(false);
            }
            if(this.interaccionTecla.isUp && !this.getArmado()){
                this.setAtaqueDistancia(false);
            }
            if(this.getInteraccion() && this.interaccionTecla.isDown){
                this.setArmado(true);
                this.setAtaqueDistancia(false);
            }
            ...
            //MOVIMIENTO
            if(this.saltarTecla.isDown && this.body.onFloor()){
                jump.play();
                this.body.setVelocityY(-400);
            }
            else if(this.defensaTecla.isDown){
                this.playerController.setState('defensa');
                this.setDefendiendo(true);
            }
            else if(this.derechaTecla.isDown){
                this.playerController.setState('moveLeft');
            }
            else if(this.izquierdaTecla.isDown){
    ...
    jugadorAtacar(){
        if(this.getStuneado() && this.getAplicarStun()){
            ...
            this.ataqueRapidoTecla.on('down', () => {
                this.setAtaqueRapido(true);
            })
            this.ataquePesadoTecla.on('down', () => {
                this.setAtaquePesado(true);
            })
            if(this.getArmado()){
                this.interaccionTecla.on('down', () => {
       ...
       if(!this.getDefendiendo()){
            if(this.ataqueRapidoTecla.isUp && this.getAtaqueRapido()){
                this.playerController.setState('ataqueRapido');
                this.setAtaqueRapido(false);
            }
            if(this.ataquePesadoTecla.isUp && this.getAtaquePesado()){
                this.playerController.setState('ataquePesado');
                this.setAtaquePesado(false);
            }
            if(this.interaccionTecla.isUp && this.getAtaqueDistancia() && this.getArmado()){
   ...
   actualizarBarraVida(vida){
    ...
    if(vida > 0){
        ...
        this.barraVida.fillRoundedRect(this.barraPos,560,380,20,2);
        this.barraVida.fillStyle(0x00ff00);    
        this.barraVida.fillRoundedRect(this.barraPos,560,0,20,2);
    }
